...
describe("groupHoursByLocalDate", () => {
  it("buckets by the local calendar day, not the UTC one", () => {
    ...
    const groups = groupHoursByLocalDate(
      [
        hour("2026-09-09T20:00:00Z"),
        hour("2026-09-09T21:00:00Z"),
        hour("2026-09-10T05:00:00Z"),
      ],
      tz,
    );
...
describe("summarizeDay", () => {
  ...
  it("totals precipitation over the hours shown rather than the daily row", () => {
    const summary = summarizeDay(
      { date: "2026-09-09", precipitation_mm: 0.2 },
      [
...
describe("buildDayChart", () => {
  ...
  it("returns null for a single hour", () => {
    expect(
      buildDayChart([hour("2026-09-10T04:00:00Z", { temperature: 10 })], local),
    ).toBeNull();
  });
  ...
  it("omits the temperature curve without temperatures but keeps the histogram", () => {
    ...
    expect(chart?.precipitation).toEqual({
      max: 1,
      bars: [],
      ticks: [
        { value: 0.5, y: 50 },
  ...
  it("anchors every slot to its hour of day so the axis is the same for every day", () => {
    ...
    expect(chart.temperature.linePath).toBe(
      `M${at(6)},70L${at(7)},50L${at(8)},30L${at(9)},10`,
    );
    expect(chart.temperature.feelsPath).toBe(
      `M${at(6)},90L${at(7)},70L${at(8)},50`,
    );
  ...
  it("labels the axis every three hours with six-hourly majors", () => {
    expect(HOUR_LABELS.map((t) => t.label)).toEqual([
      "00",
      "03",
      "06",
      "09",
    ...
    expect(HOUR_LABELS.filter((t) => t.major).map((t) => t.label)).toEqual([
      "00",
      "06",
      "12",
      "18",
