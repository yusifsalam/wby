...
- `daily_climate_normals` — per calendar day (month+day, incl. 29 Feb): temperature avg/high/low, feels-like avg/high/low (wind chill from hourly temp+wind), wind avg, daily max gust, humidity, precipitation mm, wet-day share (≥0.1 mm), snow depth, plus 24-hour UTC curves for temp/feels-like/wind/humidity. Computed from station history by `cmd/import-daily-normals` (`-from-climate-normals` covers every station with official monthly normals, ~102 of 106; stations with hourly params get the curves, from ~2000 at best, hourly precip only from 2012 so precip stays daily). Hourly-derived normals need ≥10 years of record; daily fields need half the period. Where FMI's hourly product covers a station only briefly (e.g. Pirkkala from 2019) the importer falls back to the raw 10-minute query at a one-hour step (temperature, humidity, wind; no gusts), after a one-week probe for wind. Raw FMI observations are cached as one CSV per calendar year under `data/fmi-observations/<fmisid>/{daily,hourly,hourly-instant}/<year>.csv` (gitignored; a year file is written after every fetch, empty when FMI returned nothing or the wind probe failed, so its presence alone means "fetched"), so reruns and overlapping periods fetch only the years missing on disk. Older per-period files (`daily-1991-2020.csv` etc.) are split into year files the first time a station is imported. Each row also carries the station's record length in year-equivalents (`daily_years`, `hourly_years`); `/v1/climate-normals/daily` picks the station within 50 km by distance plus 3 km per missing year of daily record, and when that station has no hourly curves for the day borrows them from the best hourly-covered station, rebased to the primary's daily mean (`hourly_station` in the response). Rows imported before migration 011 rank as zero coverage, i.e. by distance only, until re-imported. The endpoint also returns a `precipitation` block: observed mm so far today and month-to-date (hourly `PRA_PT1H_ACC` from FMI since the 1st, cached per station-hour) against the daily normals summed over the same days. The app prefers the daily card and falls back to the monthly one only where no daily normals exist.
